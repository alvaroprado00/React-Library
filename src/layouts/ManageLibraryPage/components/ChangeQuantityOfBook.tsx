...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook:any}> = (props, key) => {
    ...
    async function increaseQuantity() {
        ...
        if (!response.ok) {
            throw new Error('Error when increasing book quantity');
        }
    ...
    async function decreaseQuantity() {
        ...
        if (!response.ok) {
            throw new Error('Error when decreasing book quantity');
        }
        setQuantity(quantity - 1);
        setRemaining(remaining - 1);
    }

    async function deleteBookEnhanced() {
        ...
        if (!response.ok) {
            throw new Error('Error when deleting book');
        }
        props.deleteBook();
    }    

    return (
        <div className="card mt-3 s hadow p-3 mb-3 bg-body rounded">
            <div className="row g-0">
                ...
                <div className="mt-3 col-md-1">
                    <div className="d-flex justify-content-start">
                        <button className="m-1 btn btn-md btn-danger" onClick={deleteBookEnhanced}>Delete</button>
                    </div>
                </div>
